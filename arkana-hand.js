 ...
 *   getQuantumIndex() (con fallback a getQuantumByte()), DECKS, currentDeck, currentLang, LANG_CONFIG
 */
...
(function () {
  ...
  // ── MEDIAPIPE ──────────────────────────────────────────────────────────────
  // Versiones fijas + SRI. Para actualizar:
  //   curl -sL https://cdn.jsdelivr.net/npm/@mediapipe/hands@X.Y.Z/hands.js \
  //     | openssl dgst -sha384 -binary | openssl base64 -A
  const MP_HANDS_VERSION  = '0.4.1675469240';
  const MP_CAMERA_VERSION = '0.3.1675466862';
  const MP_HANDS_BASE     = `https://cdn.jsdelivr.net/npm/@mediapipe/hands@${MP_HANDS_VERSION}`;
  const MP_HANDS_SRI      = 'sha384-oHwoZ9HyKv5ark5VOH+XWdbNfmhYtptAOBuV8plz6mAfXvTA6d8fULuYllWouEK2';
  const MP_CAMERA_SRI     = 'sha384-q1KhAZhJcJXr3zfC3Tz07pBqQSabwFIZhXlmlUAB8s0zk4ETWERkIKGBCFQ5Jc3e';

  function loadScriptWithSRI(src, integrity) {
  ...
  async function loadMediaPipe() {
    if (window.Hands && window.Camera) return initMediaPipe();
    await loadScriptWithSRI(`${MP_HANDS_BASE}/hands.js`, MP_HANDS_SRI);
    await loadScriptWithSRI(
      `https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils@${MP_CAMERA_VERSION}/camera_utils.js`,
      MP_CAMERA_SRI
    );
  ...
  async function initMediaPipe() {
    const video = document.getElementById('handVideo');
    // `locateFile` apunta a la MISMA versión fija — los .wasm y modelos
    // tienen que coincidir con `hands.js`, si no MediaPipe falla.
  ...
  function spawnCard(nx, ny) {
    ...
    // Si index.html aún no ha cargado, fallback a un sample con sesgo
    // mínimo: rejection inline.
    const pick22 = typeof window.getQuantumIndex === 'function'
      ? () => window.getQuantumIndex(22)
      : () => { let b; do { b = window.getQuantumByte(); } while (b >= 242); return b % 22; };
    let idx, n = 0;
    do { idx = pick22(); n++; }
